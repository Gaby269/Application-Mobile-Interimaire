...
void * demandeConnexionNoeudi (void * param){ 
    ...
    struct paramsThread * args = (struct paramsThread *) param;   //recuperation des parametres
    struct predicatRdv * predicat = args -> varPartagee;          //Recuperation des variables paratgé
        //affichage
    ...
    //PRISE DU VERROU
    priseVerrou(&predicat->verrou);     //prise du verrou
    printf("[PARTICIPANT %d] Prise du verrou\n", args->idThread);

    //DECREMENTATION DU CONTEUR DE THREADS
    *(predicat->cpt) -= 1;
    ...
    while (*(predicat->cpt) != 0) {
        attentVarCond(&predicat->verrou, &predicat->condi);
        printf("[THREAD %d] J'attent les autres pour le rdv\n", args->idThread);
    }
    ...
    if (*(predicat->cpt) == 0) {
        ...
        liberationVarCond(&predicat->condi);
    }
    printf("[THREAD %d] je me reveille et je continue car tout le monde est arrivé ! \n", args->idThread);

    //LIBERATION VERROU : on a plus besoin du verrou donc on peut le liberer
    liberationVerrou(&predicat->verrou);
    printf("[THREAD %d] Liberation du verrou car plus besoin ! \n", args->idThread);
...
void * acceptationConnexionNoeudi (void * param){ 
